...
struct PIPE_CONTROL
{
    ...
    union tagTheStructure
    {
        struct tagCommon
        {
            ...
            __CODEGEN_UINT32         Reserved_60                                      : __CODEGEN_BITFIELD(28, 31)    ; ///< U4
    ...
    ///     
    ///     If the VF Cache Invalidation Enable is set to a 1 in a PIPE_CONTROL, a
    ///     separate Null PIPE_CONTROL, all bitfields sets to 0, with the VF Cache
    ...
    ///     the cache due to the fact that VF uses the VB# and the lower address
    ///     bits (address modulo 4GB) as the tag in its VB data cache. Reprogramming
    ///     the VB Start Address of a VB used in a previous draw requires a VF Cache
    ///     Invalidate, otherwise accessing the same lower address bits of the new
    ///     VB may erroneously hit in the cache (as the VB# and lower address bits
    ///     may match those used in the previous draw).
    ///     
    ...
    ///     
    ///     A PIPE_CONTROL with Command Streamer Stall Enable set must be sent prior
    ///     to enabling tessellation or a geometry shader.
    ...
    /// @details 
    ///     Ring Buffer Mode Scheduling: This field is valid only if the post-sync
    ///     operation is not 0. If this bit is set, the store data address is
    ///     actually an index into the global hardware status page. This bit only
    ///     applies to the Global HW status page. If this field is 1, the
    ///     Destination Address Type in this command must be set to 1 (GGTT).
    ///     Execlist Mode Scheduling: This field is valid only if the post-sync
    ///     operation is not 0. If this bit is set, the store data address is index
    ///     into the global hardware status page when destination address type in
    ///     the command is set to 1 (GGTT). The store data address is index into the
    ///     per-process hardware status page when destination address type in the
    ///     command is set to 0 (PPGTT).
    ///     
    ...
    ///     
    ///     Preemption (An ongoing active context being preempted by a pending
    ///     execlist) is inhibited by HW when Protected Memory Enable is set in
    ///     execlist mode of operation.
    ///     
    ///     Before setting Protected Memory Enable, SW must ensure Media engine is
    ///     woken up using PIPELINE_SELECT Force Media Wake. The Media engine must
    ///     remain awake for the duration of protection being turned on.
    ///     
    ...
    ///     
    ///     If protection is enable prior to this command then the media engine will
    ///     be forced to remain awake. To ensure Media Engine can go back to a low
...
struct MI_LOAD_REGISTER_IMM
{
    ...
    union tagTheStructure
    {
        struct tagCommon
        {
            ...
            __CODEGEN_UINT32         Reserved_13                                      : __CODEGEN_BITFIELD(13, 22)    ; ///< U10
            __CODEGEN_UINT32         MICommandOpcode                                  : __CODEGEN_BITFIELD(23, 28)    ; ///< U6
            __CODEGEN_UINT32         CommandType                                      : __CODEGEN_BITFIELD(29, 31)    ; ///< U3
