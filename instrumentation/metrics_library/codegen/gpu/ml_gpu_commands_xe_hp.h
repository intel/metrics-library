...
///     inserted after events of interest. SW is entirely responsible for
///     managing the ID field and addresses used by such a series of commands.
///     
...
struct MI_STORE_REGISTER_MEM
{
    ...
    union tagTheStructure
    {
        struct tagCommon
        {
            ...
            __CODEGEN_UINT32         WorkloadPartitionIDOffsetEnable                  : __CODEGEN_BITFIELD(16, 16)    ; ///< U1
            __CODEGEN_UINT32         MMIORemapEnable                                  : __CODEGEN_BITFIELD(17, 17)    ; ///< U1
            __CODEGEN_UINT32         Reserved_18                                      : __CODEGEN_BITFIELD(18, 18)    ; ///< U1
    ...
    __CODEGEN_INLINE void Init( void )
    {
        ...
        //TheStructure.Common.DWordLength                            = DWORD_LENGTH_EXCLUDES_DWORD_0_1;
        //TheStructure.Common.WorkloadPartitionIDOffsetEnable        = 0x0;
        //TheStructure.Common.MMIORemapEnable                        = 0x0;
        //TheStructure.Common.AddCSMMIOStartOffset                   = 0x0;
        //TheStructure.Common.PredicateEnable                        = 0x0;
        //TheStructure.Common.UseGlobalGTT                           = 0;
    ...
    /// @brief Write WorkloadPartitionIDOffsetEnable
    /// @details 
    ///     
    ///     
    ///       
    ///     This bit controls the memory write address computation for the store
    ///     data update. The final memory write address is computed by adding the  
    ///     Workload Partition ID times the "Address Offset"to the memory address
    ///     mentioned in the command. Workload Partition ID gets programmed in the
    ...
    ///     
    ///       
    ///     Example: {Final Memory Write Address[47:2], 2'b00} = (  Workload
    ///     Partition ID* Address Offset) + {Memory Write Address [47:2], 2'b00}
    ///       
    ///     
    ///     This bit controls the memory write address computation for the store
    ///     data update. The final memory write address is computed by adding
    ///     theWorkload Partition IDtimes the "Address Offset"to the memory address
    ///     mentioned in the command.Workload Partition IDgets programmed in the
    ///     WPARIDregisterand the Address Offset gets programmed through
    ///     CS_MI_ADDRESS_OFFSET register.
    ///       Example: {Final Memory Write Address[64:2], 2'b00} = (Workload
    ///     Partition ID* Address Offset) + {Memory Write Address [64:2], 2'b00}
    ///     
    ...
    /// @brief Read WorkloadPartitionIDOffsetEnable
    ...
    /// @brief Write MMIORemapEnable
    /// @details 
    ///     This bit provides a mechanism in HW to remap the MMIO address in the MI
    ///     command to the engine instance on which the command is getting executed,
    ///     remapping in HW is done using engine specific remap table. Render and
    ...
    ///     Decode and Video Enhancement engine class.  A MMIO remapping table per
    ///     engine class is created with MMIO address belonging to multiple
    ///     instances of an engine within an engine class. However Render and
    ///     Compute engine share a common remapping table to facilitate remapping
    ///     across engines, where as a dedicated remap table for each of Video
    ///     Decode and Video Enhancement engine class.  This mode provides mechanism
    ///     for SW to always use MMIO address belonging to fixed instance (instance
    ///     zero) with in an engine class during command buffer creation agnostic to
    ///     the instance on which it will get scheduled. This will also allow
    ///     context interoperability across instances with in an engine class and
    ///     extends to across engines in case of Render and Compute.
    ///     
    ...
    ///     [63:48] are ignored by the HW and assumed to be in correct canonical
    ///     form [63:48] == [47].
    ///     
...
///     
///     Following mode bits must not be set while programming the PIPE_CONTROL
///     command:
///     
///     TBIMR Force Batch Closure
///     
///     Tile Cache Flush Enable
///     
///     Depth Stall Sync Enable
///     
///     PSS Stall Sync Enable
///     
///     Depth Stall Enable
///     
...
///     
///     VF Cache InvalidationEnable
///     
///     Stall at Pixel Score Board
///     
///     Depth Cache Flush Enable
///     
///     L3 Read Only Cache Invalidate
///     
///     Texture Cache Invalidate
///     
///     CCS Flush
///     
///     L3 Fabric Fence Flush
///     
///     DC Flush Enable
///     
///     
...
///     
///     
///       In debug mode of operation with Enable Physical L3 Cache Flush bit set
///     in CS_DEBUG_MODE1 register, HW will honor DC Flush Enable bit in
///     PIPE_CONTROL and will result in flushing of physical L3 cache.
///     
///       
///     
struct PIPE_CONTROL
{
    ...
    union tagTheStructure
    {
        struct tagCommon
        {
            /// DWORD 0
            __CODEGEN_UINT32         DWordLength                                      : __CODEGEN_BITFIELD( 0,  7)    ; ///< U8
            __CODEGEN_UINT32         PredicateEnable                                  : __CODEGEN_BITFIELD( 8,  8)    ; ///< U1
            __CODEGEN_UINT32         DataportFlush                                    : __CODEGEN_BITFIELD( 9,  9)    ; ///< U1
            __CODEGEN_UINT32         L3ReadOnlyCacheInvalidationEnable                : __CODEGEN_BITFIELD(10, 10)    ; ///< U1
            __CODEGEN_UINT32         UnTypedDataPortCacheFlush                        : __CODEGEN_BITFIELD(11, 11)    ; ///< U1
            __CODEGEN_UINT32         Reserved_12                                      : __CODEGEN_BITFIELD(12, 12)    ; ///< U1
            __CODEGEN_UINT32         CompressionControlSurface_CCSFlush               : __CODEGEN_BITFIELD(13, 13)    ; ///< U1
            __CODEGEN_UINT32         WorkloadPartitionIDOffsetEnable                  : __CODEGEN_BITFIELD(14, 14)    ; ///< U1
            __CODEGEN_UINT32         SFSyncStall                                      : __CODEGEN_BITFIELD(15, 15)    ; ///< U1
            ...
            __CODEGEN_UINT32         PSSStallSyncEnable                               : __CODEGEN_BITFIELD(17, 17)    ; ///< U1
            __CODEGEN_UINT32         TLBInvalidate                                    : __CODEGEN_BITFIELD(18, 18)    ; ///< U1
            __CODEGEN_UINT32         DepthStallSyncEnable                             : __CODEGEN_BITFIELD(19, 19)    ; ///< U1
            ...
            __CODEGEN_UINT32         AMFSFlushEnable                                  : __CODEGEN_BITFIELD(25, 25)    ; ///< U1
            ...
            __CODEGEN_UINT32         L3FabricFlush                                    : __CODEGEN_BITFIELD(30, 30)    ; ///< U1
            __CODEGEN_UINT32         TBIMRForceBatchClosure                           : __CODEGEN_BITFIELD(31, 31)    ; ///< U1
    ...
    {
        TBIMR_FORCE_BATCH_CLOSURE_NO_BATCH_CLOSURE                       = 0, ///< SF will not close the batch on receiving marker.
        TBIMR_FORCE_BATCH_CLOSURE_CLOSE_BATCH                            = 1, ///< SF will close the batch on receiving the marker associated with this command.
    ...
    __CODEGEN_INLINE void Init( void )
    {
        ...
        //TheStructure.Common.PredicateEnable                        = 0x0;
        //TheStructure.Common.DataportFlush                          = 0x0;
        //TheStructure.Common.L3ReadOnlyCacheInvalidationEnable      = 0x0;
        //TheStructure.Common.UnTypedDataPortCacheFlush              = 0x0;
        //TheStructure.Common.CompressionControlSurface_CCSFlush     = 0x0;
        //TheStructure.Common.WorkloadPartitionIDOffsetEnable        = 0x0;
        //TheStructure.Common.SFSyncStall                            = 0x0;
        ...
        //TheStructure.Common.PSSStallSyncEnable                     = 0;
        //TheStructure.Common.TLBInvalidate                          = 0x0;
        //TheStructure.Common.DepthStallSyncEnable                   = 0;
        ...
        //TheStructure.Common.AMFSFlushEnable                        = 0;
        ...
        //TheStructure.Common.L3FabricFlush                          = 0x0;
    ...
    /// @details 
    ///     If set, this command is executed (or not) depending on the current value
    ///     of the MI Predicate internal state bit (MI_PREDICATE_RESULT). This
    ///     command is ignored (NOOP'd) if PredicateEnable is set and the Predicate
    ///     state (MI_PREDICATE_RESULT[0])) bit is 0.  This command is
    ///     un-conditionally NOOP'd when MI_SET_PREDICATE_RESULT[0] is set.
    ///     
    ...
    /// @brief Write DataportFlush
    /// @details 
    ///     If set, Dataport ensures its pipeline is flushed and the memory
    ///     transactions are coherent in L3$ as part of the flush operation. This
    ///     will not result in flushing L3$ potion that caches DC writes.
    ///     
    ...
    /// @brief Write L3ReadOnlyCacheInvalidationEnable
    /// @details 
    ///     This bit controls the invalidation of the L3 Read Only Cache at the top
    ///     of the pipe, i.e. at command parsing time. Setting this bit is
    ///     independent of any other bit in this packet.
    ///     
    ///     This bit controls the invalidation of the Geometry streams cached in L3
    ///     Cache at the top of the pipe, i.e. at command parsing time. Setting this
    ///     bit is independent of any other bit in this packet.
    ///     
    ...
    /// @brief Read L3ReadOnlyCacheInvalidationEnable
    ...
    /// @brief Write UnTypedDataPortCacheFlush
    /// @details 
    ///     This bit controls the flusing of the data-port's Un-Typed data cache.
    ///       If set, dataport ensures all the dirty lines in un-typed data cache af
    ///     flushed to memory and are coherent in L3 cacheas part of the flush
    ///     operation.
    ///     
    ///     
    ///     "Dataport Flush" bit must be set.
    ///     
    ///     
    ///     
    ///     This bit is functional and must be only set for GPGPU workloads, i.e
    ///     when PIPELINE_SELECT command has set Pipeline Select mode set to GPGPU.
    ///     
    ...
    /// @brief Read UnTypedDataPortCacheFlush
    ...
    /// @brief Write CompressionControlSurface_CCSFlush
    /// @details 
    ///           This bit controls the flushing of the engine (Render, Compute)
    ///     specific entries from the compression cache.      
    ///     
    ///     If a CCS$ flush is not required for coherency(CCS$ is memory side), HW
    ...
    /// @brief Read CompressionControlSurface_CCSFlush
    ...
    ///     data update. The final memory write address is computed by adding the  
    ///     Workload Partition ID times the Address Offset to the memory address
    ///     mentioned in the command. Workload Partition ID gets programmed through
    ///     WPARIDregister and the Address Offset gets programmed through
    ///     CS_MI_ADDRESS_OFFSET register..
    ///       
    ///       
    ...
    ///     data update. The final memory write address is computed by adding
    ///     theWorkload Partition IDtimes the Address Offset to the memory address
    ///     mentioned in the command.Workload Partition IDgets programmed through
    ///     WPARIDregister and the Address Offset gets programmed through
    ///     CS_MI_ADDRESS_OFFSET register..
    ///       
    ///       Example: Final Memory Write Address[47:2] = (Workload Partition ID*
    ...
    /// @brief Write SFSyncStall
    /// @details 
    ///     If set, SF Unit will stall successive workload processing until all the
    ///     prior workload is complete. Once all SF unit's are synced up (across
    ///     Slices), post-sync operations take place and then SF units will get
    ///     uninstalled.
    ///     
    ...
    ///     
    ///     If a L3$ flush is not required for coherency(L3$ is memory side), HW
    ///     will ignore this bit. Flush will be enabled
    ///     throughFF_SLICE_CS_CHICKEN1-Enable Physical L3 Cache Flush debug mode
    ///     bit even if cache is memory side.
    ///     
    ...
    /// @brief Write PSSStallSyncEnable
    /// @details 
    ///     If set, PSS Units will stall successive PS threads from being dispatched
    ///     until all the prior PS threads complete. Once all PSSs are synced up
    ///     (across Slices), post-sync operations take place and then PSS units will
    ///     get uninstalled.
    ///     
    ...
    /// @brief Write DepthStallSyncEnable
    /// @details 
    ///     If set, 3D pipeline will stall any subsequent primitives at the Depth
    ///     Test stage until they Sync across all the slices. Once all the Depth
    ///     Test Stages are synced up (across Slices), post-sync operations take
    ///     place and then they get uninstalled.
    ///     
    ...
    /// @brief Read DepthStallSyncEnable
    ...
    ///     flushes internal cache(s) to memory.  This bit should be enabled when a
    ///     procedural texture transitions from the write state to the read state. 
    ///     
    ...
    ///     
    ///     
    ///     Tile Cache Enabled Mode:SW must always set CS Stall bit when Tile Cache
    ///     Flush Enable bit is set in the PIPECONTROL command.
    ///     SW must ensure level1 depth and color caches are flushed prior to
    ///     flushing the tile cache. This can be achieved by following means:Single
    ...
    ///     rendering commands between the two PIPECONTROL commands.
    ///     
    ///       Unified Cache (Tile Cache Disabled): In unified cache mode of
    ///     operation Color and Depth (Z) streams are cached in DC space of L2 along
    ///     with Data Port stream. On a Tile Cache Flush only Color and Depth (Z)
    ///     streams from DC space of L2 are flushed to globally observable and where
    ///     as DC Flush Enable will only flush Data Port stream from the DC space of
    ///     L2 to globally observable. Refer L3 configuration section for Unified
    ///     cache usage model. In this mode of operation there is no dedicated
    ///     memory allocated for Tile Cache in L2. When the Color and Depth (Z)
    ///     streams are enabled to be cached in the DC space of L2, Software must
    ///     use Render Target Cache Flush Enable and Depth Cache Flush Enable along
    ///     with Tile Cache Flush for getting the color and depth (Z) write data to
    ///     be globally observable. In this mode of operation it is not required to
    ///     set CS Stall upon setting Tile Cache Flush bit.
    ///     
    ...
    ///     
    ///     
    ///       Pipelined L3 Fabric Flush:    When Depth Stall Enable is set L3 Fabric
    ///     Flush is pipelined along with the workload and issued from raster stage
    ///     in the pipeline. Flush marker for this flush type on reaching raster
    ///     stage, will ensure all the prior workload is complete and then L3 Fabric
    ///     Flush is performed before allowing the workload following the flush
    ///     marker to execute.    By default L3 Fabric Flush happens top of the pipe
    ///     as part of post-synchronization operation on a flush completion. SW must
    ///     explicitly set WM_CHICKEN2[5] (5584h) bit to enable Pipelined L3 Fabric
    ///     Flush. This bit must be programmed for all contexts and hence it can be
    ///     initialized during golden context creation time.    Usages:    This
    ///     mechanism is used to flush the updated compressed control state of an
    ///     surface during fast clear to global observable point before the
    ...
    ///     sections for more details.    - For a sequence of color fast clears  <li
    ///     style="margin: 0in 0in 8pt 0.5in;">A single PIPE_CONTROL command with
    ///     Render Target Cache Flush, L3 Fabric Flush and Depth Stall set at the
    ///     end of the sequence suffices. This assumes the output of the fast clears
    ///     are used only post this PIPE_CONTROL command. 
    ///         - For a sequence of depth fast clears  <li style="margin: 0in 0in
    ///     8pt 0.5in;">A single PIPE_CONTROL command with Depth Cache Flush, L3
    ///     Fabric Flush and Depth Stall set at the end of the sequence suffices.
    ///     This assumes the output of the depth fast clears are used only post this
    ///     PIPE_CONTROL command. 
    ///         - For a sequence of mixed color/depth fast clears.  <li
    ///     style="margin: 0in 0in 8pt 0.5in;">A single PIPE_CONTROL command with
    ///     Depth Cache Flush, Render Target Cache Flush,L3 Fabric Flush and Depth
    ///     Stall set at the end of the sequence suffices. This assumes the output
    ///     of the color/depth fast clears are used only post this PIPE_CONTROL
    ///     command.
    ///     
    ///     
    ...
    /// @brief Write TBIMRForceBatchClosure
    /// @details 
    ...
    /// @brief Read TBIMRForceBatchClosure
...
struct MI_STORE_DATA_IMM
{
    ...
    union tagTheStructure
    {
        struct tagCommon
        {
            ...
            __CODEGEN_UINT32         ForceWriteCompletionCheck                        : __CODEGEN_BITFIELD(10, 10)    ; ///< U1
            __CODEGEN_UINT32         WorkloadPartitionIDOffsetEnable                  : __CODEGEN_BITFIELD(11, 11)    ; ///< U1
            __CODEGEN_UINT32         Reserved_12                                      : __CODEGEN_BITFIELD(12, 20)    ; ///< U9
    ...
    __CODEGEN_INLINE void Init( void )
    {
        ...
        //TheStructure.Common.ForceWriteCompletionCheck              = 0x0;
        //TheStructure.Common.WorkloadPartitionIDOffsetEnable        = 0x0;
    ...
    /// @brief Write ForceWriteCompletionCheck
    /// @details 
    ...
    /// @brief Read ForceWriteCompletionCheck
    ...
    ///     WPARIDregister and the Address Offset gets programmed through
    ///     CS_MI_ADDRESS_OFFSET register.
    ///     Example for store dword/qword: {Final Memory Write Address[47:2], `b00}
    ///     = (  Workload Partition ID* "Address Offset" + {Memory Write Address
    ///     [47:2],`b00}
    ///     
    ///     
    ///     This bit controls the memory write address computation for the store
    ...
    ///     WPARIDregister and the Address Offset gets programmed through
    ///     CS_MI_ADDRESS_OFFSET register.
    ///       Example for store dword/qword: {Final Memory Write Address[63:2],
    ///     `b00} = (Workload Partition ID* "Address Offset" + {Memory Write Address
    ///     [63:2],`b00}
    ///     
...
///     offset into Memory-Mapped Register Range).  Any offset that is to a
///     destination outside of the GT core will allow the parser to continue
///     once the cycle is at the GT boundry and not destination.  Any other
///     address will ensure the destination is updated prior to parsing the next
///     command
///     
...
///     command to a NOOP.
///     
///     The following addresses should NOT be used for LRIs:  0x8800 - 0x88FF
///     >= 0xC0000
...
///     is exempted. Listed below are the exempted registers.  3DPRIM_END_OFFSET
...
struct MI_LOAD_REGISTER_IMM
{
    ...
    union tagTheStructure
    {
        struct tagCommon
        {
            ...
            __CODEGEN_UINT32         Reserved_13                                      : __CODEGEN_BITFIELD(13, 16)    ; ///< U4
            __CODEGEN_UINT32         MMIORemapEnable                                  : __CODEGEN_BITFIELD(17, 17)    ; ///< U1
            __CODEGEN_UINT32         Reserved_18                                      : __CODEGEN_BITFIELD(18, 18)    ; ///< U1
            __CODEGEN_UINT32         AddCSMMIOStartOffset                             : __CODEGEN_BITFIELD(19, 19)    ; ///< U1
    ...
    ///     for SW to always use MMIO address belonging to fixed instance (instance
    ///     zero) with in an engine class during command buffer creation agnostic to
    ///     the instance on which it will get scheduled. This willalso allow context
    ///     interoperability across instances with in an engine class and extends to
    ///     across engines in case of Render and Compute.
    ///     
    ///     SW must always use MMIO address belonging to Instance-0 of an engine
    ///     while enabling "MMIO Remap" in MI commands.
...
///     and writes that value to a destination register location.  Any offset
///     that is to a destination outside of the GT core will allow the parser to
///     continue once the cycle is at the GT boundry and not destination. Any
///     other address will ensure the destination is updated prior to parsing
///     the next command  
///     
...
struct MI_LOAD_REGISTER_REG
{
    ...
    ///     for each of Video Decode and Video Enhancement engine class.  A MMIO
    ///     remapping table per engine class is created with MMIO address belonging
    ///     to multiple instances of an engine within an engine class. However
    ...
    ///     Video Decode and Video Enhancement engine class.  This mode provides
    ///     mechanism for SW to always use MMIO address belonging to fixed instance
    ///     (instance zero) with in an engine class during command buffer creation
    ///     agnostic to the instance on which it will get scheduled. This willalso
    ///     allow context interoperability across instances with in an engine class
    ///     and extends to across engines in case of Render and Compute.
    ///     
    ///     SW must always use MMIO address belonging to Instance-0 of an engine
    ///     while enabling "MMIO Remap" in MI commands.
    ///     MMIO Remapping will be done by HW prior to doing any other functionaliy
    ...
    ///     table for each of Video Decode and Video Enhancement engine class.  A
    ///     MMIO remapping table per engine class is created with MMIO address
    ...
    ///     facilitate remapping across engines, where as a dedicated remap table
    ///     for each of Video Decode and Video Enhancement engine class.  This mode
    ///     provides mechanism for SW to always use MMIO address belonging to fixed
    ///     instance (instance zero) with in an engine class during command buffer
    ///     creation agnostic to the instance on which it will get scheduled. This
    ///     willalso allow context interoperability across instances with in an
    ///     engine class and extends to across engines in case of Render and
    ///     Compute.
    ///     
    ///     SW must always use MMIO address belonging to Instance-0 of an engine
    ///     while enabling "MMIO Remap" in MI commands.
